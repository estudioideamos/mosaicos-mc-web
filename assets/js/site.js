...
// Smooth wheel movement uses native document scrolling, so sticky elements keep working.
(() => {
  const enabled = matchMedia('(hover: hover) and (pointer: fine) and (prefers-reduced-motion: no-preference)');
  let frame = 0, target = 0, position = 0, written = 0, previousTime = 0, direction = 0;
  const stop = () => { cancelAnimationFrame(frame); frame = 0; direction = 0; };
  const locked = () => document.querySelector('dialog[open]') ||
  ...
  const tick = time => {
    if (!enabled.matches || locked() || Math.abs(scrollY - written) > 2) { stop(); return; }
    const elapsed = Math.min(48, Math.max(1, time - previousTime));
    previousTime = time;
    target = Math.max(0, Math.min(limit(), target));
    position += (target - position) * (1 - Math.exp(-elapsed / 155));
    const finished = Math.abs(target - position) < .6;
    if (finished) position = target;
    window.scrollTo({top: position, behavior: 'instant'});
    written = scrollY;
    if (finished) stop(); else frame = requestAnimationFrame(tick);
  };
  window.addEventListener('wheel', event => {
    if (!enabled.matches || event.defaultPrevented || !event.cancelable ||
        event.ctrlKey || event.metaKey || event.altKey || event.shiftKey ||
        !event.deltaY || Math.abs(event.deltaX) > Math.abs(event.deltaY) || locked()) { stop(); return; }
    // Let editable controls and independently scrollable panels retain native behavior.
    for (const node of event.composedPath()) {
      if (!(node instanceof Element) || node === document.body || node === document.documentElement) continue;
      if (node.matches('input, textarea, select, [contenteditable]:not([contenteditable="false"]), [data-native-scroll]') ||
          (node.scrollHeight > node.clientHeight + 1 && /^(auto|scroll|overlay)$/.test(getComputedStyle(node).overflowY))) { stop(); return; }
    }
    const delta = event.deltaY * (event.deltaMode === 1 ? 16 : event.deltaMode === 2 ? innerHeight : 1);
    const nextDirection = Math.sign(delta);
    if (!frame || nextDirection !== direction) { target = position = written = scrollY; }
    target = Math.max(0, Math.min(limit(), target + delta * .65));
    if (!frame && Math.abs(target - scrollY) < .6) return;
    event.preventDefault();
    direction = nextDirection;
    if (!frame) { previousTime = performance.now(); frame = requestAnimationFrame(tick); }
  }, {passive: false});
  // Direct actions take priority over pending wheel momentum.
