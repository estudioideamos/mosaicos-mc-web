(() => {
 ...
 const slides=[...root.querySelectorAll('.mc-cinema__slide')],video=root.querySelector('video'),pause=root.querySelector('[data-cinema-pause]'),motion=matchMedia('(prefers-reduced-motion: reduce)');
 let index=0,paused=motion.matches,timer,visible=true,failed=false;
 ...
 const sync=()=>{
  ...
  if(index!==0||!playing())video.pause();
  if(!playing())return;
  if(index===0&&!failed){video.play().catch(()=>{failed=true;sync();});}
  else timer=setTimeout(()=>show(index+1),7000);
 };
 const show=i=>{
  ...
  root.querySelector('[data-cinema-count]').textContent=String(index+1).padStart(2,'0')+' / 06';
 ...
 video.addEventListener('ended',()=>{if(index===0&&playing())show(1);});
